...
- 升级 Mocha 至 12.0.0，并使用 c8 替代 nyc 生成覆盖率，刷新相关开发依赖后
  `npm audit` 不再报告已知漏洞；这些依赖仅用于仓库开发和测试，不进入发布包的运行时依赖。

### 兼容性

- 最低支持的运行时扩展至 Node.js 12.22.0，并同时支持 CommonJS 与原生 ESM 包根导入；仓库开发与测试使用 `.node-version` 固定的
  Node.js 20.20.2，工具链要求 `^20.19.0 || >=22.12.0`，开发依赖不会随发布包安装。
- TypeScript 编译目标调整为 ES2019，并移除命令执行路径对 `AbortController` 的依赖，
  使用 Node.js 12 支持的 `child_process.exec` 原生超时机制。

### 持续集成

- 开发测试矩阵覆盖 Node.js 20、22、24，并对真实 npm tarball 使用精确的 Node.js 12.22.0
  验证严格引擎安装、CommonJS、原生 ESM、磁盘信息与系统概览。

### 验证

- 通过 TypeScript 构建、ESLint 检查、完整测试与覆盖率测试：390 passing。
- 使用官方 Node.js 12.22.0 对真实发布 tarball 完成安装和运行时冒烟测试，发布包保持零运行时依赖。
