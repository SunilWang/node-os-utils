      ...
      cp.exec('ip -s link', { shell: true }, function (err, out) {
        ...
        for (i = 0; i < stats.length; i++) {
          ...
          if (metrics[stats[i].interface]) {
            ...
            metrics[stats[i].interface] = (output.toFixed(2) + 'MB/s')
            metrics[stats[i].interface] = (input.toFixed(2) + 'MB/s')
        ...
        if (stats.length > 0) {
          ...
          metrics.total['input'] = totalOut.toFixed(2) + 'MB/s'
          totalIn = 0
          totalOut = 0
