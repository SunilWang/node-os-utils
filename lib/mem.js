...
var getMeminfo = co.wrap(function* () {
  try {
    // yield exec('cat /proc/meminfo | head -5')
    var meminfoStr = yield readFile('/proc/meminfo')
    return meminfoStr.toString().split('\n').slice(0, 5).join('\n')
  } catch (error) {
    return bucket.options.NOT_SUPPORTED_VALUE
  }
})
...
bucket.mem = {
  info: co.wrap(function* () {
    ...
    if (bucket.isNotSupported(res)) {
      ...
      freeMem =
        (parseInt(resultMemory[1], 10) +
          parseInt(resultMemory[3], 10) +
          parseInt(resultMemory[4], 10)) *
        1024
    }
...
bucket.mem.info().then(doc => console.log(doc))
